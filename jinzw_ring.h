 ...
 * for offsetof()
 */
#include "jinzw_general.h"

/**
 * @defgroup jinzw_ring Ring Macro Implementations
 * @ingroup JINZW 
 ...
 * <pre>
 *      struct my_element_t {
 *          JINZW_RING_ENTRY(my_element_t) link;
 *          int foo;
 *          char *bar;
 *      };
 * </pre>
 ...
 * than one JINZW_RING_ENTRY field. Each JINZW_RING_ENTRY has a corresponding
 * JINZW_RING_HEAD declaration.
 *
 * @warning For strict C standards compliance you should put the JINZW_RING_ENTRY
 ...
 * <pre>
 *      JINZW_RING_HEAD(my_ring_t, my_element_t);
 ...
 */
#define JINZW_RING_HEAD(head, elem)					\
    struct head {							\
	struct elem * volatile next;					\
	struct elem * volatile prev;					\
    }
 ...
 * to the start of each element, not to the JINZW_RING_ENTRY structure.
 *
 * <pre>
 *     +->+------+<-+  +->+------+<-+  +->+------+<-+
 ...
 * </pre>
 *
 * The JINZW_RING_HEAD is nothing but a bare JINZW_RING_ENTRY. The prev
 ...
 * name for its JINZW_RING_ENTRY in each element, and has its own type
 * for its JINZW_RING_HEAD.
 *
 * Note also that if the offset is non-zero (which is required if an
 * element has more than one JINZW_RING_ENTRY), the unreality of the
 * sentinel may have bad implications on very perverse implementations
 * of C -- see the warning in JINZW_RING_ENTRY.
 *
 * @param hp   The head of the ring
 * @param elem The name of the element struct
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_SENTINEL(hp, elem, link)				\
 ...
 * @param hp   The head of the ring
 */
#define JINZW_RING_FIRST(hp)	(hp)->next
/**
 * The last element of the ring
 * @param hp   The head of the ring
 */
#define JINZW_RING_LAST(hp)	(hp)->prev
/**
 * The next element in the ring
 * @param ep   The current element
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_NEXT(ep, link)	(ep)->link.next
/**
 * The previous element in the ring
 * @param ep   The current element
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_PREV(ep, link)	(ep)->link.prev
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_INIT(hp, elem, link) do {				\
	JINZW_RING_FIRST((hp)) = JINZW_RING_SENTINEL((hp), elem, link);	\
	JINZW_RING_LAST((hp))  = JINZW_RING_SENTINEL((hp), elem, link);	\
    } while (0)
 ...
 */
#define JINZW_RING_EMPTY(hp, elem, link)					\
    (JINZW_RING_FIRST((hp)) == JINZW_RING_SENTINEL((hp), elem, link))
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_ELEM_INIT(ep, link) do {				\
	JINZW_RING_NEXT((ep), link) = (ep);				\
	JINZW_RING_PREV((ep), link) = (ep);				\
    } while (0)
 ...
 *   empty rings... see JINZW_RING_SPLICE_HEAD for one that does
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_SPLICE_BEFORE(lep, ep1, epN, link) do {		\
	JINZW_RING_NEXT((epN), link) = (lep);				\
	JINZW_RING_PREV((ep1), link) = JINZW_RING_PREV((lep), link);	\
	JINZW_RING_NEXT(JINZW_RING_PREV((lep), link), link) = (ep1);	\
	JINZW_RING_PREV((lep), link) = (epN);				\
    } while (0)
 ...
 *   empty rings... see JINZW_RING_SPLICE_TAIL for one that does
 * @param lep  Element in the ring to splice after
 * @param ep1  First element in the sequence to splice in
 * @param epN  Last element in the sequence to splice in
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_SPLICE_AFTER(lep, ep1, epN, link) do {			\
	JINZW_RING_PREV((ep1), link) = (lep);				\
	JINZW_RING_NEXT((epN), link) = JINZW_RING_NEXT((lep), link);	\
	JINZW_RING_PREV(JINZW_RING_NEXT((lep), link), link) = (epN);	\
	JINZW_RING_NEXT((lep), link) = (ep1);				\
    } while (0)
 ...
 *   empty rings... see JINZW_RING_INSERT_HEAD for one that does
 * @param lep  Element in the ring to insert before
 * @param nep  Element to insert
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_INSERT_BEFORE(lep, nep, link)				\
	JINZW_RING_SPLICE_BEFORE((lep), (nep), (nep), link)
 ...
 *   empty rings... see JINZW_RING_INSERT_TAIL for one that does
 * @param lep  Element in the ring to insert after
 * @param nep  Element to insert
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_INSERT_AFTER(lep, nep, link)				\
	JINZW_RING_SPLICE_AFTER((lep), (nep), (nep), link)
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_SPLICE_HEAD(hp, ep1, epN, elem, link)			\
	JINZW_RING_SPLICE_AFTER(JINZW_RING_SENTINEL((hp), elem, link),	\
			     (ep1), (epN), link)
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_SPLICE_TAIL(hp, ep1, epN, elem, link)			\
	JINZW_RING_SPLICE_BEFORE(JINZW_RING_SENTINEL((hp), elem, link),	\
			     (ep1), (epN), link)
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_INSERT_HEAD(hp, nep, elem, link)			\
	JINZW_RING_SPLICE_HEAD((hp), (nep), (nep), elem, link)
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_INSERT_TAIL(hp, nep, elem, link)			\
	JINZW_RING_SPLICE_TAIL((hp), (nep), (nep), elem, link)
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_CONCAT(h1, h2, elem, link) do {			\
	if (!JINZW_RING_EMPTY((h2), elem, link)) {			\
	    JINZW_RING_SPLICE_BEFORE(JINZW_RING_SENTINEL((h1), elem, link),	\
				  JINZW_RING_FIRST((h2)),			\
				  JINZW_RING_LAST((h2)), link);		\
	    JINZW_RING_INIT((h2), elem, link);				\
	}								\
    } while (0)
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_PREPEND(h1, h2, elem, link) do {			\
	if (!JINZW_RING_EMPTY((h2), elem, link)) {			\
	    JINZW_RING_SPLICE_AFTER(JINZW_RING_SENTINEL((h1), elem, link),	\
				  JINZW_RING_FIRST((h2)),			\
				  JINZW_RING_LAST((h2)), link);		\
	    JINZW_RING_INIT((h2), elem, link);				\
	}								\
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_UNSPLICE(ep1, epN, link) do {				\
	JINZW_RING_NEXT(JINZW_RING_PREV((ep1), link), link) =		\
		     JINZW_RING_NEXT((epN), link);			\
	JINZW_RING_PREV(JINZW_RING_NEXT((epN), link), link) =		\
		     JINZW_RING_PREV((ep1), link);			\
    } while (0)
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_REMOVE(ep, link)					\
    JINZW_RING_UNSPLICE((ep), (ep), link)
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_FOREACH(ep, head, elem, link)                          \
    for (ep = JINZW_RING_FIRST(head);                                     \
         ep != JINZW_RING_SENTINEL(head, elem, link);                     \
         ep = JINZW_RING_NEXT(ep, link))
 ...
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_FOREACH_SAFE(ep1, ep2, head, elem, link)               \
    for (ep1 = JINZW_RING_FIRST(head), ep2 = JINZW_RING_NEXT(ep1, link);    \
         ep1 != JINZW_RING_SENTINEL(head, elem, link);                    \
         ep1 = ep2, ep2 = JINZW_RING_NEXT(ep1, link))

/* Debugging tools: */

#ifdef JINZW_RING_DEBUG
#include <stdio.h>
#include <assert.h>

#define JINZW_RING_CHECK_ONE(msg, ptr)					\
	fprintf(stderr, "*** %s %p\n", msg, ptr)

#define JINZW_RING_CHECK(hp, elem, link, msg)				\
	JINZW_RING_CHECK_ELEM(JINZW_RING_SENTINEL(hp, elem, link), elem, link, msg)

#define JINZW_RING_CHECK_ELEM(ep, elem, link, msg) do {			\
	...
	do {								\
	    ...
	    fprintf(stderr, "\telem->next %p\n",			\
		    JINZW_RING_NEXT(here, link));				\
	    fprintf(stderr, "\telem->prev %p\n",			\
		    JINZW_RING_PREV(here, link));				\
	    fprintf(stderr, "\telem->next->prev %p\n",			\
		    JINZW_RING_PREV(JINZW_RING_NEXT(here, link), link));	\
	    fprintf(stderr, "\telem->prev->next %p\n",			\
		    JINZW_RING_NEXT(JINZW_RING_PREV(here, link), link));	\
	    if (JINZW_RING_PREV(JINZW_RING_NEXT(here, link), link) != here) { \
		fprintf(stderr, "\t*** elem->next->prev != elem\n");	\
		break;							\
	    }								\
	    if (JINZW_RING_NEXT(JINZW_RING_PREV(here, link), link) != here) { \
		fprintf(stderr, "\t*** elem->prev->next != elem\n");	\
		break;							\
	    }								\
	    here = JINZW_RING_NEXT(here, link);				\
	} while (here != start);					\
	fprintf(stderr, "*** ring check end\n");			\
    } while (0)

#define JINZW_RING_CHECK_CONSISTENCY(hp, elem, link)			\
	JINZW_RING_CHECK_ELEM_CONSISTENCY(JINZW_RING_SENTINEL(hp, elem, link),\
					elem, link)

#define JINZW_RING_CHECK_ELEM_CONSISTENCY(ep, elem, link) do {		\
	struct elem *start = (ep);					\
	struct elem *here = start;					\
	do {								\
	    assert(JINZW_RING_PREV(JINZW_RING_NEXT(here, link), link) == here); \
	    assert(JINZW_RING_NEXT(JINZW_RING_PREV(here, link), link) == here); \
	    here = JINZW_RING_NEXT(here, link);				\
	} while (here != start);					\
    } while (0)
 ...
 *   (This is a no-op unless JINZW_RING_DEBUG is defined.)
 ...
 * an assertion if any inconsistency is found.  Same idea as JINZW_RING_CHECK()
 ...
 * @param msg  Descriptive message
 */
#define JINZW_RING_CHECK_ELEM(ep, elem, link, msg)
/**
 ...
 * is found.  Same idea as JINZW_RING_CHECK_ELEM() except that it's silent
 * if all is well.
 *   (This is a no-op unless JINZW_RING_DEBUG is defined.)
 * @param ep   The element
 * @param elem The name of the element struct
 * @param link The name of the JINZW_RING_ENTRY in the element struct
 */
#define JINZW_RING_CHECK_ELEM_CONSISTENCY(ep, elem, link)
#endif
